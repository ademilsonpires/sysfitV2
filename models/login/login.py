...
def autenticar_usuario(cnpj_cpf, login, senha, host=MYSQL_HOST):
    ...
    session = get_session_cliente(
        get_engine_cliente(db_name, user, password, host))
    try:
        usuario = session.query(Usuario).filter(
            Usuario.USR_LOGIN == login).first()
